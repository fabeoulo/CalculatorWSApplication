...
<html>
    ...
    <head> 
        ...
        <title>Cell - ${initParam.pageTitle}</title>
        ...
        <script>
            const startProcessingConfirmMessage = "<fmt:message key="cell.dialog.step1.startProcessingMessage" />";
            const tableLogoutConfirmMessage = "<fmt:message key="cell.dialog.step1.tableLogoutMessage" />";
            const changeTableConfirmMessage = "<fmt:message key="cell.dialog.step1.changeTableMessage" />";
            const assyOrTestCheckMessage = "<fmt:message key="cell.dialog.assyOrTestCheckMessage" />";
            ...
            const cookieInfoDataSavedMessage = "<fmt:message key="cell.cookie.DataSavedMessage" />";

            var userInfoCookieName = "userInfo", testLineTypeCookieName = "testLineTypeCookieName", cellCookieName = "cellCookieName";
            var STATION_LOGIN = "LOGIN", STATION_LOGOUT = "LOGOUT", CHANGE_STATION = "CHANGE_STATION";
//            var savedTable, savedJobnumber;
            ...
            $(document).ready(function () {
                ...
                console.log($.cookie(cellCookieName));
                ...
                $("#changeDeck").click(function () {
                    if (confirm(changeTableConfirmMessage)) {
                        modifyTestTable(CHANGE_STATION);
                    }
                });
            ...
            function checkExistCookies() {
                ...
                if (babLineTypeCookie || testLineTypeCookie) {
                    lockAllUserInput();
                    showMsg(assyOrTestCheckMessage);
                    return false;
                }
            ...
            function setLineOptions(line) {
                if ($("#userSitefloorSelect").val() === line.floor.name && "${userLinetype}".indexOf(line.lineType.name) !== -1) {
                    $("#table").append("<option value=" + line.id + " " + (line.lock == 1 ? "disabled style='opacity:0.5'" : "") + ">" + line.name + "</option>");
                }
            }
            ...
            function getLine() {
                ...
                $.ajax({
                    type: "Get",
                    url: "<c:url value="/CellStationController/findBySitefloor" />",
                    data: {
            ...
            function login(data) {
                $.ajax({
                    type: "Post",
                    url: "<c:url value="/CellLoginRecordController/login" />",
                    data: data,
                    dataType: "html",
                    success: function (response) {
                        if (response == "success") {
            ...
            function logout(data) {
                $.ajax({
                    type: "Post",
                    url: "<c:url value="/CellLoginRecordController/logout" />",
                    data: data,
                    dataType: "html",
                    success: function (response) {
                        if (response == "success") {
            ...
            function changeDesk(data) {
                $.ajax({
                    type: "Post",
                    url: "<c:url value="/CellLoginRecordController/changeDesk" />",
                    data: data,
                    dataType: "html",
                    success: function (response) {
                        if (response == "success") {
    ...
    <body>
        ...
        <div class="Div0" id="inputpannel">
            <div class="Div1 form-inline">
                ...
                <select id="table" ${key == null ?"":"disabled"} value="${tb == null ?"":tb}">
                    <option value="-1">
                        <fmt:message key="cell.select.step1.tableNo" />
                    </option>
                </select>
                ...
                <input type="button" value="<fmt:message key="cell.btn.step1.changeDesk" />" id="changeDeck">
            </div>
            <div class="Div2">
                <p>
                <h3>
                    <fmt:message key="cell.label.step1.title" />
                </h3>
                <fmt:message key="cell.label.step1.content" />
                </p>
            </div>
        </div>
        <div class="Div0"> 
            <div class="Div1">
                <font id="smsg">
                <fmt:message key="cell.label.step2.serverMessageTitle" />
                </font>
            ...
            <div class="Div2">
                <p>
                <h3>
                    <fmt:message key="cell.label.step2.title" />
                </h3>
                <fmt:message key="cell.label.step2.content" />
                </p>
            </div>
        </div>
        ...
        <div id="totaldata">
            <div class="Div0">
                ...
                <div class="Div2">
                    <h4>
                        <fmt:message key="cell.label.step3.content" />
                    </h4>
                </div>
            </div>
<!--            <div class="Div0">
            ...
            <div class="Div0">
                ...
                <div class="Div2">
                    <p>
                    <h3>
                        <fmt:message key="cell.label.step5.title" />
                    </h3>
                    <fmt:message key="cell.label.step5.content" />
                    </p>
                </div>
            </div>
        </div>
